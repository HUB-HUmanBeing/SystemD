...
Template.topicHeader.helpers({
    ...
    isMemberNotified: function () {
        let membersToNotify = Template.currentData().currentSpreadsheet.membersToNotify
...
Template.topicHeader.events({
    ...
    'submit [editSpreadsheetForm]': function (event, instance) {
        event.preventDefault()
        let topic = Spreadsheet.findOne(FlowRouter.current().queryParams.topicId)
        let topicName = event.target.editSpreadsheetName.value
    ...
    'click [toggleNotifications]': function (event, instance) {
        event.preventDefault()
        $('.tooltipped').tooltip('remove')
        let topic =Spreadsheet.findOne(instance.data.currentSpreadsheet._id)
        let projectId = FlowRouter.current().params.projectId
