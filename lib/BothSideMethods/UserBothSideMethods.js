...
User.extend({
    meteorMethods: {
        ...
        /*******************
         * remise a zéro du compteur de messages non lus
         * @param convId
         */
        conversationRead(convId){
            //on check l'id de la conversation
            check(convId, String)
            //on récupere l'utilisateur courant
            let user = User.findOne({_id : Meteor.userId()})
            //et on passe a zéro la bonne ligne
            user.profile.conversations.forEach((conv,i)=>{
                if(conv.conversation_id === convId){
                    user.profile.conversations[i].unreadMessage = 0
                }
            })
            //enfin on sauvegarde
            user.save()
