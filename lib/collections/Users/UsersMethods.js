...
Meteor.methods({
    ...
    updateSelfProfile: function (fieldName, value) {
       // on check que le field name est bien une chaine
        ...
        check(value, UserSelfProfileSchema.pick(fieldName) );
        //puis on lance l'upsert'

        Meteor.users.upsert( {_id : Meteor.userId()},{
            $set : {"profile.public[fieldName]" : value[fieldName]}
        });
    },
    //methode de l'update dela la localisation utilisateur
    updateSelfLocation: function (value) {

        //on check  que l'objet value correspond bien au format de location
        check(value, locationSchema );
        //puis on lance l'upsert'

        Meteor.users.upsert( {_id : Meteor.userId()},{
            $set : {"profile.public.location" : value}
        });
    }
});